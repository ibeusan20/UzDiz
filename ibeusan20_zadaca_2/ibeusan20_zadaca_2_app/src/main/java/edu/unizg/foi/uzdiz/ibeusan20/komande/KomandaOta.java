...
public class KomandaOta implements Komanda {
  ...
  public KomandaOta(UpraviteljAranzmanima uprAranz, UpraviteljRezervacijama uprRez,
      String... argumenti) {
  ...
  public boolean izvrsi() {
    ...
    // dohvat svih rezervacija tog aranžmana
    List<Rezervacija> sveRez = uprRez.dohvatiZaAranzmanIVrste(oznaka, "PAČOOD");

    if (sveRez.isEmpty()) {
      a.postaviOtkazan();
      ispis.ispisi("Turistički aranžman " + oznaka + " je otkazan (nije bilo rezervacija).");
      return true;
    }

    boolean biloOtkaza = false;
    for (Rezervacija r : sveRez) {
      boolean uspjelo = uprRez.otkaziRezervaciju(r.getIme(), r.getPrezime(), oznaka);
      if (uspjelo) {
        biloOtkaza = true;
      }
    }
    ...
    if (biloOtkaza) {
      ispis.ispisi("Uspješno otkazan turistički aranžman " + oznaka
          + " i otkazane pripadajuće rezervacije.");
    } else {
      ispis.ispisi("Nije pronađena nijedna rezervacija za aranžman " + oznaka
